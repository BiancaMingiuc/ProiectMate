...
def analizeaza_date_mate():
    ...
    plt.title('Total Raspunsuri Corecte vs Gresite per Elev')
    plt.ylabel('Numar Răspunsuri')
    ...
    if not df_elev.empty:
        ...
        procentaj = df_elev['scor_corecte']/(df_elev['scor_corecte']+df_elev['scor_gresite'])*100 
        plt.plot(df_elev['created_at'], procentaj, 
                 marker='o', label='Corecte', color='green')
        plt.fill_between(df_elev['created_at'], procentaj, color='green', alpha=0.1)
        
        plt.title(f'Progresul elevului {nume} în timp')
        plt.xlabel('Data')
        plt.ylabel('Procentaj Corecte')
        plt.ylim(0, 105)
        ...
        print("Nu exista suficiente date.")
